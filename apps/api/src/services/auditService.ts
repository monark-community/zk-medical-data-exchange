...
class AuditService {
  ...
  async getBlockedStudiesForUser(
    userAddress: string,
    studyIds: number[]
  ): Promise<Map<number, { blocked: boolean; reason?: string }>> {
    const result = new Map<number, { blocked: boolean; reason?: string }>();
    
    if (process.env.NODE_ENV === "test") {
      studyIds.forEach(id => result.set(id, { blocked: false }));
      return result;
    }

    for (const studyId of studyIds) {
      const cacheKey = `${userAddress.toLowerCase()}_${studyId}`;
      const cachedFailure = AuditService.failedApplicationCache.get(cacheKey);
      
      if (cachedFailure) {
        result.set(studyId, {
          blocked: true,
          reason: "Previous application failed eligibility or verification checks",
        });
      } else {
        result.set(studyId, { blocked: false });
      }
    }
    ...
    try {
      ...
      for (const actionId of actions) {
        try {
          ...
          const [timestamp, blockNumber, user, userProfile, actionType, resource, action, dataHash, success, metadata] = recordData;

          if (
            Number(actionType) === ActionType.STUDY_PARTICIPATION &&
            !success
          ) {
            const match = resource?.match(/^study_(\d+)$/);
            if (match) {
              const studyId = parseInt(match[1]);
              
              if (studyIds.includes(studyId)) {
                const reason = "Previous application failed eligibility or verification checks";
                const cacheKey = `${userAddress.toLowerCase()}_${studyId}`;
                
                AuditService.failedApplicationCache.set(cacheKey, {
                  timestamp: Date.now(),
                  reason,
                });
                
                result.set(studyId, { blocked: true, reason });
              }
            }
          }
        } catch (recordError) {
          logger.warn(
            { error: recordError, actionId: Number(actionId) },
            "Failed to read audit record"
          );
          continue;
        }
      }
      ...
      logger.error(
        { error, userAddress },
        "Failed to check for blocking failed applications"
      );
      return result;
    }
  }

  async hasBlockingFailedApplication(
    userAddress: string,
    studyId: string
  ): Promise<{ blocked: boolean; reason?: string }> {
    const studyIdNum = parseInt(studyId);
    const results = await this.getBlockedStudiesForUser(userAddress, [studyIdNum]);
    return results.get(studyIdNum) || { blocked: false };
  }

  markApplicationAsFailed(userAddress: string, studyId: string, reason: string): void {
    const cacheKey = `${userAddress.toLowerCase()}_${studyId}`;
    AuditService.failedApplicationCache.set(cacheKey, {
      timestamp: Date.now(),
      reason,
