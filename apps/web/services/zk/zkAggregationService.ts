  ...
  async fetchStudyBins(studyContractAddress: string): Promise<StudyBins> {
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ [ZK-AGG] ============================================');
        console.warn('⚠️ [ZK-AGG] Contract does not support bins (legacy deployment)');
        console.warn('⚠️ [ZK-AGG] Error:', binError.message);
        console.warn('⚠️ [ZK-AGG] Using default bins for backward compatibility');
        console.warn('⚠️ [ZK-AGG] ============================================');
        
        // Return default bins for all metrics
        const defaultBins = this.getDefaultBins();
        console.log('📊 [ZK-AGG] Default bins:', defaultBins);
        return defaultBins;
      }

      // Convert from contract format to TypeScript format
      console.log('🔄 [ZK-AGG] Converting contract bins to TypeScript format...');
