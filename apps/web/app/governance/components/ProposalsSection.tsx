...
export default function ProposalsSection({
  ...
  const { proposals, isLoading, error, refetchProposals } =
    variant === "my" ? myProposalsData : allProposalsData;
  ...
  // Stats specific to "my" variant
  const canVoteProposals = proposals.filter((p) => p.state === 0 && !p.hasVoted).length;
  const yourVotesCount = proposals.filter((p) => p.hasVoted).length;
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-white tracking-tight">
                    {variant === "my" ? "Your Proposals" : "All Proposals"}
                  </h3>
                    ...
                    <>
                      ...
                      <div className="bg-white/10 backdrop-blur-sm px-4 py-2 rounded-lg border border-white/20">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="text-xs text-blue-100">Can Vote For</div>
                            <div className="text-lg font-bold text-white">{canVoteProposals}</div>
                          </div>
                        </div>
                      </div>
                      <div className="bg-white/10 backdrop-blur-sm px-4 py-2 rounded-lg border border-white/20">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="text-xs text-blue-100">Your Votes</div>
                            <div className="text-lg font-bold text-white">{yourVotesCount}</div>
                          </div>
                        </div>
                      </div>
                    </>
