 ...
 *
 * @param walletKey - The user's wallet key as a Uint8Array.
 * @returns A 64-character hexadecimal string representing the 256-bit AES key.
 */
export function generateAESKey(walletKey: Uint8Array): string {
  const saltBytes = new TextEncoder().encode(Config.APP_SALT);
  const combined = new Uint8Array(walletKey.length + saltBytes.length);

  combined.set(walletKey);
  combined.set(saltBytes, walletKey.length);
 ...
 * @param key - The AES key as a Buffer or string.
 ...
 *
 * @param encrypted - Base64 string returned by encryptWithKey.
